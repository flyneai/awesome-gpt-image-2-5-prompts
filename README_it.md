# Prompt per GPT Image 2.5 — Flyne AI

<!-- BEGIN FLYNE ENTRY -->
**[Prova GPT Image 2.5 gratis · Senza registrazione](https://flyne.ai/free-gpt-image-2-5/)**

Una immagine di riferimento; massimo 2.000 caratteri. Controlla i requisiti di ogni ricetta. Gratis e senza registrazione è l’offerta dichiarata dal sito; la generazione senza accesso non è stata verificata. [→](docs/flyne-access.md)

106 ricette, 16 raccolte e 142 PNG originali, inclusi gli esempi ereditati da FLAQ. 76 ricette bilingui EN/ZH, 18 in inglese e 12 specifiche per lingua. Questa pagina è introduttiva.
<!-- END FLYNE ENTRY -->

Edizione open source curata da Flyne AI, basata sulla raccolta FLAQ. Crediti e registri di generazione originali sono conservati. [FLAQ → Flyne](docs/upstream.md)

[English](README.md) · [简体中文](README_zh.md) · [繁體中文](README_tw.md) · [日本語](README_ja.md) · [한국어](README_ko.md) · [Español](README_es.md) · [Français](README_fr.md) · [Deutsch](README_de.md) · [Português (Brasil)](README_pt.md) · **Italiano** · [Русский](README_ru.md) · [العربية](README_ar.md) · [हिन्दी](README_hi.md) · [ไทย](README_th.md) · [Bahasa Indonesia](README_id.md) · [Tiếng Việt](README_vi.md)

[![Flyne AI — GPT Image 2.5](assets/previews/cover.jpg)](assets/images/cover.png)


**[Galleria delle 106 ricette illustrate](docs/gallery.md)**

## Come iniziare

Scegli una ricetta nell’[indice](prompts/README.md). Per modificare un’immagine, allega i riferimenti indicati e distingui ciò che deve cambiare da ciò che va conservato. Controlla il risultato e usa la versione approvata come base per la modifica successiva.

## Prova in italiano

[↗ L005](prompts/11-multilingual.md#l005)

```text
Crea un poster verticale 2:3 per una caffetteria immaginaria. Su carta color crema, mostra una tazza di ceramica blu con caffè e un cucchiaino, illuminati dalla luce laterale del mattino. In alto scrivi esattamente "UN CAFFÈ, SENZA FRETTA", sotto "Concediti una pausa" e in basso "CAFFÈ DI QUARTIERE". Usa caratteri blu scuro, margini ampi e una gerarchia chiara. Conserva gli accenti e la virgola, senza aggiungere prezzi o indirizzi.
```

Controlla e salva la prima immagine. Allegala ed esegui questa modifica separatamente.

```text
Nella modifica successiva cambia soltanto il colore della tazza in verde oliva, mantenendo testo, ombre e composizione.
```

Controlla l’accento in CAFFÈ, la virgola e l’assenza di parole aggiuntive.

## Strumenti IA GPT Image 2.5 gratuiti, senza registrazione

**[Flyne AI · 100% Free · No Signup Required](https://flyne.ai/free-gpt-image-2-5/)**

## Funzioni avanzate: controlla il costo in crediti prima di generare

[GPT Image 2.5 — Flare / Sunburst](https://flyne.ai/model/gpt-image-2-5/)

[Gratis e senza registrazione è l’offerta dichiarata dal sito; la generazione senza accesso non è stata verificata.](docs/flyne-access.md)

## Stato degli esempi

Questa pagina adatta L005 all’italiano. La variante italiana non è stata generata separatamente; l’esempio collegato appartiene alla ricetta spagnola originale. [L005](prompts/11-multilingual.md#l005).

Le immagini provengono dallo strumento Codex senza ID del modello disponibile; non sono test verificati di Flare / Sunburst.

## Informazioni su Flyne AI

Edizione open source curata da Flyne AI, basata sulla raccolta FLAQ. Crediti e registri di generazione originali sono conservati.

[Flyne AI](https://flyne.ai) · [GPT Image 2.5](https://flyne.ai/model/gpt-image-2-5/)

## Risorse e contributi

[106 prompts](prompts/README.md) · [Before / after](docs/launch-examples.md) · [Generation log](docs/generation-log.md) · [OpenAI API guide](docs/api-guide.md) · [Contributing](CONTRIBUTING.md)

Questo progetto non è affiliato a OpenAI né approvato da OpenAI. [MIT License](LICENSE) © 2026 Flaq AI.

[X community: 6 English prompts, original posts and source image previews](prompts/15-x-community.md)

[Nuovi prompt e video Flyne](prompts/16-flyne-x-discoveries.md) · [▶](docs/x-videos.md)

## Collaborazioni di affiliazione

Flyne AI accoglie partner affiliati. [Scopri il programma e come candidarti](https://flyne.ai/affiliate-program/).
